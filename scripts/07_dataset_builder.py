...
# ============================================================
# 时间戳提取工具（论文式3.4-0c，供时间戳编码使用）
# ============================================================

# 港股交易时段 → 时段ID映射（与 08_model_trainer.py 中 NUM_HOUR_BINS=4 对齐）
# 0: 开盘(9:30-10:00), 1: 早盘(10:00-12:00), 2: 午盘(13:00-14:30), 3: 尾盘(14:30-16:00)
def _map_hour_bin(ts: pd.Timestamp) -> int:
    """将时间戳映射到港股日内时段ID"""
...
def extract_time_ids(df: pd.DataFrame, ts_col: str = 'ts') -> Tuple[np.ndarray, np.ndarray]:
    """
    从DataFrame的时间戳列中提取时段ID和星期ID。

    返回的数组与df行一一对应，可在滑动窗口切片后
    与特征序列一起传入模型的TimeStampEncoding模块。
    ...
    Returns:
        hour_ids:    (N,) int32, 取值 0-3 (日内时段)
        weekday_ids: (N,) int32, 取值 0-4 (周一=0 ~ 周五=4)
    """
    ts = pd.to_datetime(df[ts_col])
    hour_ids = np.array([_map_hour_bin(t) for t in ts], dtype=np.int32)
    weekday_ids = ts.dt.weekday.values.astype(np.int32)  # Monday=0, Friday=4
    return hour_ids, weekday_ids
