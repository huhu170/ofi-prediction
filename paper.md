...
> **[TODO: 表 4.1-1]** 样本与特征统计汇总
> - 脚本：`exp_4_1_sample_feature_stats.py`
> - 内容：样本概况 + 特征分布 + 标签分布

表4.1-1汇报了各标的的样本量、数据完整性及核心特征的描述性统计。结果显示：收益率分布呈现典型的尖峰厚尾特征（峰度>3），成交不平衡（TI）分布近似对称，三分类标签比例接近均衡。

### 二、特征解释力分析

本节通过相关性检验与回归分析，验证各类特征对收益率的解释能力。

> **[TODO: 表 4.1-2]** 特征解释力分析
> - 脚本：`exp_4_1_feature_analysis.py`
> - 内容：相关性检验 + OLS回归 + 多尺度R²对比

> **[TODO: 图 4.1-1]** 特征分布与解释力可视化
> - 脚本：`exp_4_1_feature_analysis.py`

表4.1-2的回归结果显示，成交不平衡（TI）与短期动量特征对同期收益具有显著解释力。多尺度特征融合的R²显著高于任一单尺度，验证了假设H1a。
